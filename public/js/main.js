const numberInput = document.getElementById('number'),
      ...
      response = document.querySelector('.response');//class
...
const fetchServer = ({ number, text }) => {  //fetch api//to get response
  console.log('send');
  fetch('/', {
    method: 'post',
    headers: {
    ...
    body: JSON.stringify({ number, text }) //json string
  })
    .then(function (res) {
      console.log(res);
    })
...
function send() {
  const number = numberInput.value.replace(/\D/g, '');//no non numeric characters
